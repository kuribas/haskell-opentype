{-# LANGUAGE LambdaCase #-}
module Truetype.Fileformat.Post where
...
putPostTable table = do
  ...
  when (postVersion table == PostTable2) $ do
    putWord16be $ fromIntegral $ length (glyphNameIndex table) + 258
    traverse_ (putWord16be.fromIntegral) $ glyphNameIndex table
    for_ (postStrings table) $ \str -> do
